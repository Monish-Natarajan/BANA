...
#dict such that input coco cat --> voc cat
    ...
    with torch.no_grad():
        for it, (img, bboxes, bg_mask) in enumerate(tqdm(train_loader)):
            ...
            for uni_cls in gt_labels_with_Bg:
                ...
                orig_cat_id=0#weights are present for Background Class too!
                if uni_cls!=0:
                  orig_cat_id = trainset.cat_id_map.index(uni_cls)
                if indices.sum()==0 and orig_cat_id in coco_to_voc_map:
                    voc_id = coco_to_voc_map[orig_cat_id][1]
